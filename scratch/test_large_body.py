...
def test_large_body():
    ...
    # Create a large dummy prompt (around 130KB)
    large_text = "a" * (130 * 1024)
    payload = {
        "model": "llama-3.3-70b-versatile",
        "messages": [
            {"role": "user", "content": f"Hello! Respond with exactly the word 'OK'. Here is some dummy data: {large_text}"}
        ]
    }
    ...
    print(f"[TEST] Sending large request ({len(data)} bytes) to Cloudflare Worker proxy...")
    try:
        with urllib.request.urlopen(req, timeout=20) as res:
            print(f"[TEST] Success! Response status: {res.status}")
            body = res.read().decode('utf-8')
            print(f"[TEST] Response body: {body[:300]}")
